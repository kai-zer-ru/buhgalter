<script lang="ts">
	...
	import { getCredit, type Credit } from '$lib/api/client';
	...
	async function load() {
		...
		try {
			...
			autoDebitEnabled = Boolean((c.debit_time_local ?? '').trim());
			loadError = null;
		} catch (err) {
...
<PageLoadGate {loading} error={loadError} onretry={() => void load()} inline>
	{#if credit}
		<FormPageShell title={$_('credits.modal.autoPaymentTitle')} backHref={returnTo} onback={finish}>
			<div class="space-y-4">
