<script lang="ts">
	...
	const interceptDraftParam = $derived($page.url.searchParams.get('intercept_draft'));
		...
		(() => {
			const draftId =
				typeof window !== 'undefined'
					? new URLSearchParams(window.location.search).get('intercept_draft')
					: null;
			if (!draftId) return null;
			const draft = getInterceptDraft(draftId);
			return draft ? prefillFromDraft(draft) : null;
		})();
	const type = $derived(
