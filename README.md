<p align="center">
  <img src=".images/logo.png" alt="Бухгалтер" width="480">
</p>
...
- **Счета и операции** — наличные, банковские и кредитные карты; группы «Мои средства» и «Кредитные средства»; архивация и мягкое удаление с переводом остатка; автопополнение банковского счёта по порогу (перевод сразу после триггерной операции, в ленте выше неё). Категории и подкатегории с иконками (создание и правка). Доходы, расходы и переводы — в поле суммы можно складывать и вычитать (`7899+500`). Комиссия при переводе или списании с кредитки привязана к операции и отдельно не редактируется. Плановые операции, повтор из меню строки, периодические списания, **шаблоны** для частых ручных платежей. На операции — **магазин** и **теги**; в выпадающих списках счета в том же порядке, что на главной.
- **Кредитные карты** — лимит, погашение переводом, списание комиссии; смена лимита — отдельным действием «Изменить лимит» (не в форме редактирования счёта).
...
- **Подписки** — отдельный экран для сервисов: сводка «в месяц», три ближайшие даты списания (можно править), привязка истории; учитываются в плановом остатке вместе с периодическими.
...
- **Импорт и экспорт** — Cubux (только операции) и нативный CSV «Бухгалтер» для полного переноса учёта между серверами (счета, справочники, операции, кредиты, долги, подписки и остальное). В профиле — **«Удалить данные»**: сброс учёта без удаления логина.
...
- **Android-клиент** — приложение к вашему инстансу: вход по логину/паролю или API-токену, офлайн-очередь, синхронизация операций с сервером (в т.ч. правок из веба), виджеты, share-intent, поиск сервера в LAN (mDNS), PIN и биометрия, **перехват push и SMS банков и платёжных приложений** → черновик операции или перевод. APK — в [GitHub Releases](https://github.com/kai-zer-ru/buhgalter/releases). Подробнее — [Android-клиент](#android-клиент) и [docs/android-client.md](docs/android-client.md).
...
- Счета, операции, переводы, долги, кредиты (пошаговое создание), подписки, бюджет, статистика, шаблоны, магазины/теги, импорт/экспорт, настройки, админка (без управления пользователями и без переключателей модулей — модули только в web).
- Офлайн: очередь изменений (операции, переводы, категории, долги в т.ч. погашение, счета, бюджет, действия по кредитам без создания, периодические, подписки) с replay при появлении сети; справочники (категории, магазины, теги и др.) и карточки разделов (счета, должники, кредиты) хранятся на устройстве и не стираются при записи.
- Синхронизация: ручной pull подтягивает с сервера добавление, правку и удаление операций (в т.ч. сделанные в вебе и не на первой странице списка) и обновляет кеш списков и карточек.
- **Перехват push и SMS банков и платёжных приложений** (opt-in): черновики расхода/дохода из уведомлений или SMS → подтверждение в форме. Кошельки (MIR Pay, Samsung Pay, СБПэй и др.) — тот же пайплайн; из пары расход+пополнение можно оформить перевод. Один банк можно привязать к нескольким счетам.
- Ввод суммы — своя цифровая клавиатура с `+` / `−`.
- Виджеты: быстрые действия, «Мои средства», бюджет, «Скоро» (кредит / долг / плановые / подписки / периодические), один счёт. Суммы скрыты, пока нужен PIN, если не включена опция «показывать виджеты при блокировке».
- «Поделиться» из других приложений открывает форму расхода; ярлыки лаунчера — расход / доход / перевод.
- Блокировка PIN + биометрия (кнопка отпечатка на клавиатуре PIN); при выходе, отключении сервера или истечении сессии блокировка сбрасывается. Штатный клон ОС (Dual Apps) не поддерживается — не запускается и не ломает основную копию. После удаления APK сессия не восстанавливается из бэкапа системы.
