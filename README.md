...
- **Импорт и экспорт** — формат Cubux и нативный CSV «Бухгалтер» (CSV/XLSX).
...
- **Android-клиент** — приложение к вашему инстансу: вход по логину/паролю или API-токену, офлайн-очередь, синхронизация операций с сервером (в т.ч. правок из веба), виджеты, share-intent, поиск сервера в LAN (mDNS), PIN и биометрия, **перехват push и SMS банков** → черновик операции. APK — в [GitHub Releases](https://github.com/kai-zer-ru/buhgalter/releases). Подробнее — [Android-клиент](#android-клиент) и [docs/android-client.md](docs/android-client.md).
...
- Счета, операции, переводы, долги, кредиты (пошаговое создание), подписки, бюджет, статистика, шаблоны, магазины/теги, настройки, админка (без управления пользователями и без переключателей модулей — модули только в web).
...
- **Перехват push и SMS банков** (opt-in): черновики расхода/дохода из уведомлений или SMS → подтверждение в форме. Один банк можно привязать к нескольким счетам.
...
Справочники по установке, данным, UI и API — [docs/README.md](docs/README.md). История изменений — [CHANGELOG.md](CHANGELOG.md). Последний релиз — [v1.5.3](docs/release-notes-v1.5.3.md).
