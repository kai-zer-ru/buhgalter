...
- **Android-клиент** — приложение к вашему инстансу: вход по логину/паролю или API-токену, офлайн-очередь, виджеты, share-intent, поиск сервера в LAN (mDNS), PIN и биометрия, **перехват push и SMS банков** → черновик операции. APK — в [GitHub Releases](https://github.com/kai-zer-ru/buhgalter/releases). Подробнее — [Android-клиент](#android-клиент) и [docs/android-client.md](docs/android-client.md).
...
- Офлайн: очередь изменений (операции, переводы, категории, долги в т.ч. погашение, счета, бюджет, действия по кредитам без создания, периодические, подписки) с replay при появлении сети; справочники (категории, магазины, теги и др.) хранятся на устройстве и обновляются, не стираются при каждой записи.
...
Справочники по установке, данным, UI и API — [docs/README.md](docs/README.md). История изменений — [CHANGELOG.md](CHANGELOG.md). Release notes v1.5.1 — [docs/release-notes-v1.5.1.md](docs/release-notes-v1.5.1.md).
