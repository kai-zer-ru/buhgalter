...
- [О проекте](#о-проекте)
- [Что нового в v1.1.0](#что-нового-в-v110)
- [Демо](#демо)
...
- Долги и кредиты с графиками платежей (редактирование сумм будущих платежей, учёт задним числом, списание ретро-платежей со счёта)
...
- Админка: пользователи, **сброс пароля**, бэкапы, диагностика, секретный ключ шифрования токенов уведомлений
- Адаптивный интерфейс для мобильных браузеров
- REST API и интерактивная документация OpenAPI

Текущая версия: **v1.1.0**. История изменений — [CHANGELOG.md](CHANGELOG.md). Планы — [ROADMAP.md](ROADMAP.md).

## Что нового в v1.1.0

| Область | Кратко |
| -------- | ------ |
| Пароль | Запрос сброса на входе; админ сбрасывает пароль и видит очередь запросов |
| Кредиты | Полный график по сроку; правка сумм неоплаченных платежей; ретро-платежи «списан со счёта»; оплаченные платежи нельзя менять |
| Переводы | Поле комиссии → системная категория «Комиссия»; редактирование перевода из списка операций |
| Статистика | Фильтры по дате без времени; сводка счёта учитывает переводы; согласованность с списком операций |
| UI | Мобильное меню, карточки, touch-targets; переключатели вместо чекбоксов; фильтр категорий по типу операции |
| Прочее | Демо-стенд, кнопка доната в админке; roadmap — сканер чеков |

Подробнее: [docs/release-notes-v1.1.md](docs/release-notes-v1.1.md).
...
Пока **внешний URL** в админке не задан, доступны **localhost** и **локальная сеть** (`10.x`, `172.16–31.x`, `192.168.x`). Публичный IP или чужой домен без `external_url` вернёт 403 — см. [Nginx](#nginx-reverse-proxy--https).
...
В админке (`/admin`) укажите **внешний URL** — например `https://buhgalter.my-site.ru` — для ссылок в уведомлениях и **доступа из интернета** через reverse proxy.
...
| `BUHGALTER_LOCALES_DIR`  | `locales` или `server/locales` | Каталог с `ru.json` и `en.json` (сообщения API и шаблоны уведомлений)                                                                                               |
...
Токены Telegram и MAX шифруются AES-256-GCM. Ключ задаётся **только в админке** (`/admin`) — сохраняется в `system_settings.notification_secret_key`, текущее значение не отображается. **Ровно 32 символа.**

## Бэкапы

В админке: **Бэкапы** (`/admin/backups`).
...
**С v1.0.0 на v1.1.0:** сделайте бэкап, замените бинарник/образ, перезапустите. Применятся миграции `020`, `023`; дополнятся графики кредитов и появится категория «Комиссия». См. [docs/release-notes-v1.1.md](docs/release-notes-v1.1.md).

**Бинарник / ручная установка:** остановите процесс, замените `buhgalter`, запустите снова. Миграции применятся при старте. Сделайте бэкап до обновления.
...
Версию приложения и предыдущую версию после обновления можно посмотреть в админке → **Диагностика**.
...
- [docs/release-notes-v1.1.md](docs/release-notes-v1.1.md) — подробности релиза v1.1.0

## API-документация

После запуска сервера (порт **8765**):
...
Авторизация: сессия (cookie) или Bearer API-токен из настроек пользователя.
