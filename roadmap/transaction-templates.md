...
На странице: список, создать / изменить / удалить, drag-and-drop `sort_order`; в меню строки — **«Создать операцию»** (prefill формы). Форма шаблона — те же контролы, что у операции (счёт, категория, сумма с возможностью оставить пустой — «Указывать сумму при создании», описание, магазин, теги). Без даты. Пустое состояние — по [ui-empty-states.md](../docs/ui-empty-states.md).

### Главная: добавление операции по шаблону (web + Android)

На главной (`/` / home) — блок **перед** спойлером бюджета ([ui-budget.md](../docs/ui-budget.md)):

1. Спойлер `CollapsibleSection` (`account-group-panel` + chevron), заголовок «Шаблоны» — в едином стиле с «Мои средства» / «Последние операции».
2. По умолчанию **свёрнут**.
3. Внутри — компактный список: имя + сумма (если задана), порядок `sort_order`.
4. Тап по строке → форма с prefill (дата «сейчас»).
5. Если шаблонов нет — блок **не показывать**.
6. Ссылки «Управлять» в спойлере **нет** — управление только из настроек.

Порядок блоков на главной (фрагмент): … → **спойлер шаблонов** → **спойлер бюджета** → …
