...
- [ ] Приходит такое сообщение: Запрос на восстановление пароля: Максим (@kaizer), время: 2026-06-30T04:22:46Z. - время должно быть в человеческом формате (2026-01-01 12:13:14). И добавить ссылку на страницу восстановления.
- [ ] Страница диагностики - время сборки тоже в человеческий формат. И взять за правило - везде время выводить ТОЛЬКО в формате 2026-01-01 12:13:14
- [ ] Окно сброса пароля (при переходе из уведомления) - не закрывавется на кнопку "Отмена"
- [ ] Долги: При создании операции время 00:00 а не то, которое сейчас. В селектор времени (скрытый) надо подставлять текущее время. И во всех окнах создания операций (кроме оплаты кредита наперёд) - время надо подставлять текущее
- [ ] Редактирование операции - время встаёт --:-- а должно быть время из базы
...
- [ ] Уведомления. За сутки до списания платежа по кредиту, периодической операции - проверять достаточно ли денег на балансе. И если не достаточно - слать уведомление. Но надо продумать этот омомент досконально. Ну либо слать если баланс ушёл в минус, указывая причину.
