...
- [ ] Многоуровневое меню сверху. Подумать что вынести ещё туда можно. Как вариант - разбить настройки на разные страницы, каждая из которых будет иметь свою ссылку в меню (Настройки), туда же добавить пункт "Админка", в меню которого все пункты из админки. Надо будет тестировать и на мобильной версии, как это будет
- [ ] Сортировка в плановых операциях на главной, от новых к старым. 
- [ ] Баг. Создаётся АПИ-токен с датой истечения в прошлом. Надо блокировать в календаре прошлые даты, включая сегодня. Это отдельным параметром в компоненте "Календарь" задаётся.
