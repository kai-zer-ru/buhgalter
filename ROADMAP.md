...
- [ ] Шаблоны сообщений - разбить на блоки и сохранять каждый отдельно
- [ ] Отдельный компонент пагинации. Кнопки "В начало", "Назад", "Вперёд", "В конец", 20 записей на страницу. Кнопки выводить по смыслу, к примеру на первой странице не выводить "В начало" и "Назад", так как это и есть начало, и назад нет перехода. Это надо занести в документацию проекта. Все страницы, где есть (или должна быть) пагинация - перевести на использование этого компонента
...
- [ ] Уведомления по операциям - проверять достаточно ли денег на балансе. И если не достаточно - добавить в текст уведомление об этом. Например: "Платёж по кредиту «{credit}»: {amount}. Дата: {when}. На балансе не хватает 1 200 рублей!", а если хватает - то обычное "Платёж по кредиту «{credit}»: {amount}. Дата: {when}". Шаблон "На балансе не хватает 1 200 рублей!" вынести в отдельный шаблон и подставлять его к сообщению.
## Общие планы
