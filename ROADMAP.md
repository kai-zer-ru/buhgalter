...
- [x] [Release notes](docs/release-notes-v1.2.3.md)
- [x] Добавить кнопку "Повторить" в списке операций (в меню ...) ([release notes](docs/release-notes-v1.2.3.md), [transactions-display](docs/transactions-display.md))
- [x] В окне перевода - при выборе счёта - удалять его из второго селекта. А то сейчас можно перевести с Яндекса на Яндекс (будет ошибка, правильно, но в UI это можно выставить) ([release notes](docs/release-notes-v1.2.3.md))
- [ ] В полях ввода не выводить 0.00 а делать placeholder. Это по всему проекту. Сейчас где-то есть он, где-то текстом 0.00, а где-то вообще пустые поля
- [ ] Наличные -> Перевод -> Счёт "Откуда" должен автоматом встать "Наличные". На остальных страницах если нажата кнопка - то счёт по умолчанию в "Откуда" вставлять
- [x] UI. Время по умолчанию в периодических операциях выставлять 8:00 по местному времени (TimeZone). ([release notes](docs/release-notes-v1.2.3.md))
- [x] Баг: периодическая операция появляется в списке операций, но баланс не изменяется ([release notes](docs/release-notes-v1.2.3.md))
- [x] Баг: Время для списания кредита установлено, к примеру, на 10:00, списывается верно, баланс изменяется. НО! Операция в базу заносится с временем 00:00 ([release notes](docs/release-notes-v1.2.3.md))
...
- [ ] Кредит (не ипотека и не рассрочка) - добавить переключатель "Кредит наличными", и если включён - сумму кредита прибавлять к текущему балансу. НО! Если кредит заполняется "задним числом" - блокировать эту функцию в отключенном состоянии и давать подсказку, что изменение баланса невозможно. 
