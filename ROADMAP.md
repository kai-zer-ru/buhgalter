...
- [x] Баг: периодическая операция появляется в списке операций, но баланс не изменяется
- [ ] Баг: Время для списания кредита установлено, к примеру, на 10:00, списывается верно, баланс изменяется. НО! Операция в базу заносится с временем 00:00
## v1.2.4
