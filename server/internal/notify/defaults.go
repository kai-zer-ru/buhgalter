...
var fallbackTemplates = map[string]map[string]string{
	"ru": {
		...
		TriggerSubscriptionCharge: "Подписка «{name}»: {amount}. Списание {when} со счёта «{account}»\n{subscription_url}",
	...
	"en": {
		...
		TriggerSubscriptionCharge: "Subscription \"{name}\": {amount}. Charge {when} from \"{account}\"\n{subscription_url}",
