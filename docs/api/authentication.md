...
| `DELETE` | `/api/v1/user/data` | Сброс учёта: удалить операции, счета, долги, кредиты и остальное; категории — как у нового пользователя. Профиль, сессии и API-токены сохраняются |
