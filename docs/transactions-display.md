...
- **Счета (v1.3):** на мобильных (`< sm`) список счетов свёрнут в спойлер «Счета (N)» (`<details class="dashboard-accounts-panel">`); на десктопе — сетка `sm:grid-cols-2` без спойлера. Карточки: `AccountIcon` (`type`, `bank_icon` из API), имя, баланс, лимит кредитки, индикатор автопополнения (`bank`), прогноз; меню «⋯» — как на `/accounts` ([ui-row-actions.md](ui-row-actions.md)).
...
- «Последние операции»: две группы в спойлерах `<details>` внутри одной карточки:
...
Общий компонент — [ui-pagination.md](ui-pagination.md). На `/transactions` и в операциях счёта: **20** записей на страницу (`limit=20`), номер страницы в URL (`?page=`). Кнопки «В начало», «Назад», «Вперёд», «В конец» всегда на месте; на первой/последней странице недоступные — `disabled`. Блок скрыт, если страница одна.
