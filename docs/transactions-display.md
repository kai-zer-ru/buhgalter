...
## Пагинация `/transactions`

`TransactionPagination`: **20** операций на страницу (`limit=20`), номер страницы в URL (`?page=`). Кнопки «В начало», «Назад», «Вперёд», «В конец»; блок скрыт, если страница одна. Тот же компонент — на странице счёта (`/accounts/[id]`).
