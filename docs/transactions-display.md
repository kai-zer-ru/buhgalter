...
**Шаблоны операций (v1.5.0)** — именованные пресеты полей для быстрого ручного создания (не scheduler). Управление: `/settings/transaction-templates` (web + Android; в меню строки — «Создать операцию»). На главной — спойлер «Шаблоны» (`CollapsibleSection`, **свёрнут по умолчанию**) в едином стиле с группами счетов / «Последние операции», **перед** спойлером бюджета (скрыт, если шаблонов нет); тап по шаблону → форма с prefill (`repeatFrom` / `?template=`). Из журнала — «Сохранить как шаблон» (те же ограничения, что у «Повторить»). API: `/api/v1/transaction-templates`. План: [transaction-templates.md](../roadmap/transaction-templates.md).
