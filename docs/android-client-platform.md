...
- **Прогрев кэша:** core при старте (без блокировки UI, батчами); детали кредитов + tx-index + subcategories — только при ручном sync (`force`). SWR-revalidate отключена на время warm (+ grace). Resume — cooldown 5 мин.
