...
6. После мутаций ref-cache сбрасывает **изменяемые** списки (дашборд, счета, операции); **словари** (categories/subcategories, merchants, tags, banks, debtors, templates, `ui/meta`) и `/auth/me` остаются на устройстве и только перезаписываются свежим GET / seed. Полный сброс — logout / отключение сервера. Профиль также в `last_user` при логине и успешном `loadUser`
