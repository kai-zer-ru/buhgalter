...
- **Прогрев кэша:** при старте (после `initAuthToken`, только если токен есть) и ручной синхронизации (`warmRefCache`) — dashboard, ui/meta, счета, списки кредитов **и детали каждого кредита**, recurring, subscriptions (+ summary), долги, бюджет, банки, типовые списки операций.
