...
Данные: JSON-снимок в EncryptedSharedPreferences через Capacitor plugin `WidgetBridge` (`publish` / `setLockEnabled` / `clear`). UI публикует снимок после `warmRefCache` и загрузки главной; выход и смена сервера очищают. Фон: WorkManager раз в 60 мин (`WidgetRefreshWorker`, OkHttp + Bearer). `AppWidgetProvider.onUpdate` только биндит RemoteViews из снимка — **не** ставит `runOnce` (иначе `updateAll` → `ACTION_APPWIDGET_UPDATE` → снова `onUpdate` зацикливает WorkManager и держит CPU ~70%).
