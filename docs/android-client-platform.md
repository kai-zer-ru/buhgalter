...
- **Перехват уведомлений банка:** `BankNotificationListenerService` + `BankSmsReceiver` + `NotificationInterceptPlugin` → JS-парсеры (allowlist package / SMS-отправителей для банков каталога) → локальные черновики **расхода или дохода**. OTP/баланс/отказы — игнор; покупки → expense; входящие (пополнение, выплата процентов, перевод от …) → income. **SMS в шторке** от Google Messages / Samsung Messages — по заголовку (отправитель, в т.ч. «Ваш Т-Банк») маппятся на банк так же, как `SMS_RECEIVED`. При открытии черновика: категория/подкатегория по истории того же типа и сматченного `merchant_id` (majority категории из последних 100, затем majority непустой подкатегории; иначе primary). Настройки: `/settings/bank-notifications` (per-user toggle, счёт→банк many-to-one, last4→счёт; блок NLS; блок SMS `RECEIVE_SMS`; автозапуск/батарея). Черновики — меню «Черновики банка»; история — `BuhgalterNotificationHistory` (+ метка канала push/SMS). Отмена покупки снимает черновик расхода; дубли push↔SMS схлопываются (сумма/магазин/last4, 2 ч). Сырые push/SMS только на устройстве. **Фон:** NLS должен жить без UI — на MIUI без автозапуска после «закрыть все» push теряется (смахнул — не восстановить). Rebind: `onResume`, `onListenerConnected`, `BOOT_COMPLETED` / `MY_PACKAGE_REPLACED`. Quick-actions в шторке — follow-up ([roadmap/notification-intercept.md](../roadmap/notification-intercept.md)).
