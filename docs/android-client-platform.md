...
- **Прогрев кэша:** core при старте (без блокировки UI, батчами); детали кредитов + tx-index + subcategories — только при ручном sync (`force`). SWR-revalidate отключена на время warm (+ grace). Resume — cooldown 5 мин. Пути списков операций — через `transactionsListPath` (query-ключи отсортированы), иначе home/warm писали в разные ключи ref-cache. Главная: `shouldSuppressHomeDataRefresh` только пока warm in-flight; после `dataRefreshTick` — soft-reload (с retry, если warm ещё идёт).
