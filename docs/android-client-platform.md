...
- **Справочники (ref-cache):** GET через `request()` кэшируются в `localStorage` (ключ включает URL сервера), **кроме** путей с `/preview` (в т.ч. `GET /budgets/spent-preview`) и **`GET /setup/status`** (флаг регистрации / bootstrap). В том числе кешируется **`GET /credits/{id}`** (тело кредита с графиком) — иначе офлайн нельзя открыть карточку. **Stale-while-revalidate:** при наличии кэша экран рисуется сразу, сеть — в фоне (`ref-cache.ts`, store `refCacheUpdate` по path, legacy `refCacheTick`). `writeRefCache` не пишет и не шлёт `refCacheUpdate`, если сырой JSON совпал с кэшем; для `/api/v1/dashboard` — сравнение без полей `*_display`. При недоступности сервера — только кэш (некэшируемые GET сразу дают miss). `GET /ui/meta` перезаписывает на устройстве словари (`categories`, merchants, tags, banks, debtors, templates); прогрев `…/subcategories` — при ручном sync / heavy warm. Словари **не стираются** при write — только обновляются; полный сброс — logout / отключение сервера. После write — `clearRefCache({ preserveAuthMe: true })` сбрасывает дашборд/операции, оставляя `/auth/me`, словари и списки счетов; при пустом `/accounts` — seed из `ui/meta`. `onCreditUpdated` патчит уже существующие списки кредитов и **не** засевает `[один кредит]` после `clearRefCache` — иначе SWR считает снимок полным, а ручной sync не ходит в сеть. `clearRefCache` сбрасывает SWR-cooldown (`lastRevalidatedAt`). `listAccounts` / `listCategories` при офлайн miss читают этот fallback. `listSubcategories` при офлайн miss возвращает `[]`, не роняя форму. **`assignIfChanged`** в `load({ background: true })` — меньше лишних reactive-прогонов; dashboard через `stableEqual` без `*_display`. Native HTTPS (`SslTrust`) выполняется в пуле потоков + JS-таймаут, чтобы не блокировать очередь CapacitorPlugins.
