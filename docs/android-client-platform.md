...
- Оба origin должны указывать на **один** инстанс сервера; ref-cache хранится отдельно по URL. Чтение ищет кэш по **любому** сохранённому origin (старый LAN IP, `_no_server` и т.п.). Справочники для форм дополнительно дублируются в **`buhgalter.offline_catalogs.v1`** (без привязки к URL, как `last_user`) при каждом `seedDictionariesFromUIMeta`; при разблокировке PIN — `reconcileOfflineCatalogsOnUnlock()` пересаживает их в ref-cache активного URL. При сворачивании приложения — `flushRefCacheDisk()`.
...
- **Справочники (ref-cache):** GET через `request()` кэшируются в `localStorage` (ключ включает URL сервера), **кроме** путей с `/preview` (в т.ч. `GET /budgets/spent-preview`) и **`GET /setup/status`** (флаг регистрации / bootstrap). Кешируются карточки **`GET /credits/{id}`**, **`GET /debtors/{id}`**, **`GET /accounts/{id}`** (+ `/balance`). **Stale-while-revalidate:** при наличии кэша экран рисуется сразу, сеть — в фоне (`ref-cache.ts`, store `refCacheUpdate` по path, legacy `refCacheTick`). `writeRefCache` не пишет и не шлёт `refCacheUpdate`, если сырой JSON совпал с кэшем; для `/api/v1/dashboard` — сравнение без полей `*_display`. При недоступности сервера — только кэш (некэшируемые GET сразу дают miss). `GET /ui/meta` перезаписывает на устройстве словари (`categories`, merchants, tags, banks, debtors, templates). Все persistable GET **не стираются** при write — следующий онлайн-GET форсирует сеть (`pendingNetworkRefresh`); полный сброс — logout / отключение сервера. При пустом `/accounts` — seed из `ui/meta`. `getDebtor` / `getDebt` / `getAccount` / `getCredit` при офлайн-miss собирают карточку из списков (`section-fallbacks.ts`) — иначе «Долги → должник» падает, если деталь никогда не прогревалась. `onCreditUpdated` патчит уже существующие списки кредитов и **не** засевает `[один кредит]`, если списка нет. `clearRefCache` сбрасывает SWR-cooldown (`lastRevalidatedAt`). `listAccounts` / `listCategories` при офлайн miss читают fallback. `listSubcategories` при офлайн miss возвращает `[]`, не роняя форму. **`assignIfChanged`** в `load({ background: true })` — меньше лишних reactive-прогонов; dashboard через `stableEqual` без `*_display`. Native HTTPS (`SslTrust`) выполняется в пуле потоков + JS-таймаут, чтобы не блокировать очередь CapacitorPlugins.
...
- **Прогрев кэша:** core при старте (без блокировки UI, батчами) — главная, словари, списки всех разделов (счета, долги, должники, кредиты, подписки, периодические, бюджет, статистика). Heavy сразу после core (тоже батчами, не только ручной sync): карточки каждого кредита / должника / счёта + `stats/context` + tx-index + subcategories. SWR-revalidate отключена на время warm (+ grace). **Force warm** (`pullFromServer`) обходит SWR и всегда ходит в сеть (`runWithForcedRefCacheNetwork`) — иначе отравленный список кредитов не чинится синхронизацией. Resume — cooldown 5 мин. Пути списков операций — через `transactionsListPath` (query-ключи отсортированы), иначе home/warm писали в разные ключи ref-cache. Главная: `shouldSuppressHomeDataRefresh` только пока warm in-flight; после `dataRefreshTick` — soft-reload (с retry, если warm ещё идёт).
...
- **Синхронизация:** ручной pull (`AndroidDrawerSync`) — кнопка всегда активна (кроме момента sync); в офлайне сначала принудительный `/health`, при успехе — прогрев кэша + replay outbox, при неудаче остаётся офлайн. После прогрева — `GET /sync/transaction-changes?since_id=` (курсор в ref-cache): события create/update/delete операций (включая долги и кредиты) патчат все закешированные списки и индекс по id — так подтягиваются добавление, правка и удаление операции за месяцы назад, которые warm страницы 1 не видит. `meta.total` лента не увеличивает для операции старше текущей страницы и для входящей ноги перевода (это та же операция): число в спойлере берётся из GET списка. `meta.total` лента не увеличивает для операции старше текущей страницы и для входящей ноги перевода (это та же операция): число в спойлере берётся из GET списка. При cooldown фонового warm лента всё равно запрашивается. Фоновый `syncOutbox` — только outbox. После успешной отправки outbox и после **успешного онлайн-write** во всех offline wrappers (`afterOnlineWrite` → `dataRefreshTick` + `scheduleSyncOutbox`) страницы перезагружают данные — иначе delete на списке оставлял бы stale state до ручного sync (пустой outbox → `scheduleSyncOutbox` no-op). Фоновое обновление кэша — через `refCacheTick`. После локального save в outbox — мгновенный re-merge через `localDataTick` / `outboxTick`. Кнопка sync в drawer **не закрывает** меню.
- **Версии:** `fetchAppVersionInfo()` сравнивает `APP_VERSION` с `current_version` сервера; сеть **не чаще 1×/сутки** (кэш результата). **Блокировка** (`VersionBlockScreen`) — только при отставании **мажор** или **минор** (`1.4.x` vs `1.5.x`, `1.x` vs `2.x`). Патч (`1.4.0` vs `1.4.1`) — предупреждение в drawer и попап по клику, без блокировки.
...
- **Перехват уведомлений банка:** `BankNotificationListenerService` + `BankSmsReceiver` + `NotificationInterceptPlugin` → JS-парсеры (allowlist package / SMS-отправителей для банков каталога **и платёжных приложений**: MIR Pay, Samsung Pay, СБПэй, Google Wallet, Huawei Wallet, Mi Pay, ЮMoney) → локальные черновики **расхода или дохода**. OTP/баланс/отказы — игнор; покупки → expense; входящие (пополнение, выплата процентов, перевод от …) → income. Пара расход+доход (одна сумма, разные счета или банки, окно 2 ч) показывается **одной** карточкой «Возможный перевод» (не тройкой карточек); форма перевода после сохранения снимает оба черновика. Счётчик в меню и на главной — число карточек (пара = 1), не сырых черновиков. Справочники счетов на этом экране грузятся в фоне и не блокируют создание/удаление. **SMS в шторке** от Google Messages / Samsung Messages — по заголовку (отправитель, в т.ч. «Ваш Т-Банк») маппятся на банк так же, как `SMS_RECEIVED`. Кошелёк не в привязке «счёт → банк»: счёт по last4 (если *XXXX* уникален) или по банку в тексте. При создании черновика: категория/подкатегория по истории того же типа и сматченного `merchant_id` (majority категории из последних 100, затем majority непустой подкатегории; иначе primary) — хранится на draft для Accept из шторки. Настройки: `/settings/bank-notifications` (per-user toggle перехвата, тумблер «Уведомления в шторке» + `POST_NOTIFICATIONS`, счёт→банк many-to-one, last4→счёт; блок NLS; блок SMS `RECEIVE_SMS`; автозапуск/батарея). Черновики — меню «Черновики банка»; история — `BuhgalterNotificationHistory` (+ метка канала push/SMS); у строк, парсящихся как расход/доход — либо «Создать операцию», либо «Создать перевод» при уникальной паре (prefill без постановки в очередь). **Quick-actions (v1.6.0):** после постановки push/SMS в очередь — сразу provisional-уведомление; если UI мёртв (и без PIN-lock) — тихий wake WebView → parse → native-зеркало (пара перевода = одно уведомление) с Accept/Reject; тап → форма; Accept online — `POST /transactions` или `/transfers`; offline → JS outbox. С PIN-lock полный Accept/Reject после разблокировки. Отмена покупки снимает черновик расхода; дубли push↔SMS и банк↔кошелёк схлопываются (сумма/магазин/last4, 2 ч). Сырые push/SMS только на устройстве. **Фон:** NLS должен жить без UI — на MIUI без автозапуска после «закрыть все» push теряется (смахнул — не восстановить). Rebind: `onResume`, `onListenerConnected`, `BOOT_COMPLETED` / `MY_PACKAGE_REPLACED`.
...
- **Показывать виджеты при блокировке** — по умолчанию выкл.; если вкл., home-screen виджеты не скрывают суммы при lock screen
...
- **Выход** (drawer), **отключение сервера** (Настройки → Сервер) и **истечение сессии** — `clearAppLock()`: PIN, биометрия, таймаут и опция виджетов удаляются без ввода старого PIN; следующий вход — настройка с нуля
...
| Баланс | «Мои средства»: наличные / банковские карты / кредитные средства | `/accounts` |
| Бюджет месяца | план / факт / прогресс | `/budget` |
| Скоро | до 5 ближайших: кредит, долг, плановые, подписки, периодические | карточка / главная |
| Один счёт | выбранный счёт (основной по умолчанию); configure — DayNight + отступы system bars | `/accounts/{id}` |

Данные: JSON-снимок в EncryptedSharedPreferences через Capacitor plugin `WidgetBridge` (`publish` / `setLockEnabled` / `clear`). UI публикует снимок после `warmRefCache` и загрузки главной; выход и смена сервера очищают. Фон: WorkManager раз в 60 мин (`WidgetRefreshWorker`, OkHttp + Bearer). Суммы во всех виджетах — как в UI (`formatBalance` / `1 500.00 ₽`). `AppWidgetProvider.onUpdate` только биндит RemoteViews из снимка — **не** ставит `runOnce` (иначе `updateAll` → `ACTION_APPWIDGET_UPDATE` → снова `onUpdate` зацикливает WorkManager и держит CPU ~70%).

Приватность: по умолчанию суммы в виджетах скрыты («Заблокировано» / `••••`), пока нужен экран PIN (`shouldHideWidgetAmounts`). После разблокировки виджеты снова показывают суммы. В Настройки → Безопасность можно включить **«Показывать виджеты при блокировке»** (`showWidgetsWhenLocked`, по умолчанию выкл.) — тогда суммы остаются видимы и при lock screen. Quick actions кликабельны всегда. Без токена — «Откройте приложение и войдите».
