...
CI (`.github/workflows/release.yml`): фронт собирается на **Node 22**, Go-бинарь — нативно на runner’е с кросс-компиляцией под целевую архитектуру (`BUILDPLATFORM` / `GOOS`/`GOARCH`); слои кэшируются через GitHub Actions Cache и тег `ghcr.io/kai-zer-ru/buhgalter:buildcache`. Экшены GitHub Actions работают на Node 24. QEMU нужен только для финального `alpine`-слоя (apk). Локально: `make docker-build`.
