...
- `commission` = 0;
- `transaction_date` = дата/время триггерной операции (той, после которой баланс упал ниже порога). Если даты нет (включение автопополнения при уже низком балансе, импорт без известной даты) — текущий момент.
