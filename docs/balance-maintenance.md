...
- `commission` = 0;
- `transaction_date` = дата/время триггерной операции **плюс 1 секунда** (автопополнение позже триггера). Если +1 с ушло бы в будущее — та же дата, что у триггера. Если даты нет (включение автопополнения при уже низком балансе, импорт без известной даты) — текущий момент.
- `created_at` тоже после триггера (+1 с). Список `date_desc` (новые сверху): автопополнение **выше** триггера. Иначе при той же секунде RFC3339Nano перевода сортируется раньше RFC3339 операции и автопополнение выглядит как более старое.
