...
| `/subscriptions/new` | Новая подписка (`?from_tx=` — префилл из операции) |
| `/subscriptions/[id]/edit` | Редактирование подписки |
...
На карточке счёта (`/accounts/[id]`) после сохранения операции `dataRefreshTick` перезагружает **счёт + баланс + список** (не только операции). Успешный `POST`/`PUT`/`PATCH`/`DELETE` в Android `client.ts` вызывает `clearRefCache({ preserveAuthMe: true })` + `invalidateApiCache`: снимки разделов **остаются** (офлайн-карточки должника/счёта/кредита не пропадают), следующий онлайн-GET идёт в сеть. Если `/accounts` пуст — seed из `ui/meta`. Онлайн-write во всех offline wrappers → `afterOnlineWrite` (`dataRefreshTick`); без этого delete/save на списке (без remount формы) оставлял stale UI до sync.
...
| Раздел | Маршрут |
|--------|---------|
| Профиль | `/settings/profile` — тема: **Как на устройстве** (`system`, по умолчанию), светлая, тёмная; `system` резолвится через `prefers-color-scheme`. Кнопка **«Удалить данные»** (только онлайн) сбрасывает учёт к состоянию нового пользователя |
...
| Банковские уведомления | `/settings/bank-notifications` (push банков и кошельков + SMS; черновики — `/settings/bank-notifications/drafts`) |
...
Управление пользователями (`/admin/users`) и переключатели модулей (`PUT /admin/features`) — только в веб-версии. Android читает `GET /features` и скрывает выключенные разделы, но не даёт их настраивать.
...
- **Выйти** слева, **версия APK** справа (`android/ui/package.json` → `__APP_VERSION__` при сборке)
...
- Экран PIN (`AppLockScreen`) — после входа, до основного UI; биометрия — иконка отпечатка в нижнем левом углу цифровой клавиатуры
- Настройка: `/settings/security` — включение, смена PIN, биометрия, **таймаут** в фоне (30 с…1 час), опция **показывать виджеты при блокировке** (по умолчанию выкл.)
