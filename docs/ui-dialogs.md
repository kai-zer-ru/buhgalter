...
| Режим | Где | Поведение |
|-------|-----|-----------|
| `visible` | Операции, переводы | Дата + поле времени в попапе |
| `hidden` | Фильтры, долги, даты кредитов | Только дата; в значении `T00:00` |
| `optional` | Редко | Дата обязательна; время — в `<details>` |
...
- После выбора даты в режиме без времени попап закрывается («Готово» или клик вне).

**Долги** (`DebtForm`): поля «Дата операции» и «Плановый возврат» — только дата; в API уходят границы суток в timezone пользователя (начало / конец дня).
