...
| Форма | Поле |
|-------|------|
| `TransactionForm` | дата операции; блок «Указать время» вынесен в спойлер опциональных полей (`showOptionalTimeUI={false}`, `bind:timeExpanded`). Исключение: режим платежа по кредиту («Кредиты») — время сразу под датой, без спойлера |
...
### День + месяц + время (без года)

Ближайшие списания на `/subscriptions`: `formatAPIDayMonthTimeForDisplay` → «4 мая в 09:22» / «May 4 at 09:22». Год не нужен: максимальный период подписки — год, до следующего списания меньше года.
