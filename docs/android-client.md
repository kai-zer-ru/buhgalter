...
Блокировка: Настройки → Безопасность — PIN, опциональная биометрия, таймаут в фоне и опция **показывать виджеты при блокировке** (по умолчанию выкл.). **Выход**, отключение сервера и истечение сессии сбрасывают блокировку — при следующем входе настраивается заново.
...
**Клон приложения** (Xiaomi «двойные приложения» и аналоги) **не поддерживается**: клон сразу показывает нативный экран и не открывает WebView — иначе падает сам и вешает основную копию. Удалите клон в настройках системы. Второй сервер — второй профиль Android или отдельный `applicationId`.

**Удаление приложения** должно выкинуть из учётки: системный бэкап сессию не восстанавливает. Если после uninstall снова виден вход и кеш — это баг (до v1.5.3 так делал Auto Backup / HyperOS).
...
**Перехват уведомлений:** opt-in чтение push и SMS банков и платёжных приложений (MIR Pay, Samsung Pay, СБПэй и др.) → черновик расхода или дохода. Уникальная пара расход+пополнение — одна карточка перевода. В шторке — **Принять** / **Отклонить** (тумблер в настройках). В **истории уведомлений** — либо «Создать операцию», либо «Создать перевод» (уникальная пара) по распарсенной строке, без повторного ввода суммы. Настройки — **Банковские уведомления**. Подробнее — [android-client-platform.md](android-client-platform.md), [notification-intercept.md](../roadmap/notification-intercept.md).

**Импорт / экспорт и сброс учёта:** Настройки → Импорт / экспорт (Cubux и «Бухгалтер»); в профиле — «Удалить данные» (только онлайн).

**Home-screen виджеты:** быстрые действия; баланс («Мои средства» → счета); бюджет; «Скоро» (кредит / долг / плановые / подписки / периодические); один счёт. Суммы скрыты, пока нужен PIN, если не включена опция «показывать виджеты при блокировке» — см. [android-client-platform.md](android-client-platform.md#home-screen-виджеты).
