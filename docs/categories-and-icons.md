...
**Системные категории** (`is_system: true`): «Долги» (income и expense, иконка `loan`, этап 4); «Кредиты» (только expense, иконка `loan`, этап 5). Seed при создании пользователя + backfill при старте БД (`categoryseed.EnsureSystemCategories`). `PUT` / `DELETE` → `403 Forbidden`; `POST /categories/{id}/subcategories` → `403 Forbidden`. В UI настроек — только просмотр, без раскрытия подкатегорий.

**Главная категория:** не более одной с `is_primary: true` на пользователя и тип (`expense` / `income`). При создании первой категории типа она становится главной. При удалении главной — главной становится первая в текущем порядке. На этапе 3 форма операции подставляет главную категорию по умолчанию.

**Порядок:** список отсортирован по `sort_order`, затем `name`. Новые категории получают `sort_order = max + 1`. Порядок меняется в настройках (↑↓) и сохраняется через `PUT /categories/order`.
